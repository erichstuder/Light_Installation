...
if __name__ == "__main__":
    ...
    if dispatcher.arguments.build_all:
        ...
        run_command([work_dir+'/run.py', '--Textual', '--simulator', '--unit_tests', '--test', '--report', '--verbose', '--pseudo_tty_off'])
        run_command([work_dir+'/run.py', '--Textual', '--simulator', '--features', '--test', '--report', '--verbose', '--pseudo_tty_off'])
        run_command([work_dir+'/run.py', '--Textual', '--doc', '--build', '--verbose', '--pseudo_tty_off'])
        run_command([work_dir+'/run.py', '--doc', '--build', '--verbose', '--pseudo_tty_off'])
    exit(0)
